...
public class Functions {
  ...
  FormattedNamedFunction<X, Q> validationQuality(
    ...
    Function<POCPopulationState<?, ?, S, Q, P>, Q> f = state -> state.problem().validationQualityFunction().apply(
        individualF.apply(state).solution());
    return FormattedNamedFunction.from(f, format, "validation.quality[%s]".formatted(NamedFunction.name(individualF)))
        .compose(beforeF);
  }
}
