...
public class ExtractionComparison extends Worker {
  ...
  public void run() {
    ...
    List<NamedFunction<? super POCPopulationState<?, ? extends Extractor<Character>, ? extends List<Double>>, ?>>
        functions = List.of(
            nOfIterations(),
            births(),
            elapsedSeconds(),
    ...
    solvers.put("dfa-hash+-speciated", p -> {
      ...
      Set<Character> positiveChars = p.qualityFunction().getDesiredExtractions().stream()
          .map(r -> (Set<Character>) new HashSet<>(
              p.qualityFunction().getSequence().subList(r.lowerEndpoint(), r.upperEndpoint())))
          .reduce(Sets::union)
          .orElse(Set.of());
      ...
      return new SpeciatedEvolver<>(
          ...
          Map.of(
              ...
              new ArcAddition<DeterministicFiniteAutomaton.State, Set<Character>>(
                  ...
                  .withChecker(checker),
              graphArcAdditionRate,
              new ArcRemoval<DeterministicFiniteAutomaton.State, Set<Character>>(s -> s.getIndex() == 0)
                  .withChecker(checker),
              graphArcRemovalRate,
