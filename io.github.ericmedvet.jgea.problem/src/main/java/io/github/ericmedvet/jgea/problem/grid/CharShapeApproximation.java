...
public class CharShapeApproximation
    implements ComparableQualityBasedProblem<Grid<Character>, Double>, ProblemWithExampleSolution<Grid<Character>> {
  ...
  public CharShapeApproximation(String syntheticTargetName, boolean translation, boolean smoothed, boolean weighted)
      throws IOException {
  ...
  public Function<Grid<Character>, Double> qualityFunction() {
    return grid -> {
      ...
      if (smoothed) {
        sum = sum / 2d
            + GridUtils.hammingDistance(grid.map(c -> c == null ? null : true), smoothedTarget, translation)
                / 2d;
      }
      if (weighted) {
