...
public record ListNumericalDataset(
  ...
  public ListNumericalDataset {
    ...
    if (!examples.isEmpty()) {
      if (xsSizes.size() > 1) {
        throw new IllegalArgumentException("Size of x is not consistent across examples, found sizes %s".formatted(
            xsSizes));
      }
      if (ysSizes.size() > 1) {
        throw new IllegalArgumentException("Size of y is not consistent across examples, found sizes %s".formatted(
            ysSizes));
      }
  ...
  protected static List<Map<String, String>> loadFromCSV(InputStream inputStream, long limit) throws IOException {
    try (inputStream) {
