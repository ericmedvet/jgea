...
public record ListNumericalDataset(
  ...
  public ListNumericalDataset {
    ...
    if (!examples.isEmpty()) {
      ...
      if (xVarNames.size() != xsSizes.get(0)) {
        throw new IllegalArgumentException(
            ("Number of names of x vars is different form size of x in examples: %d vs"
                    + " "
                    + "%d")
      ...
      if (yVarNames.size() != ysSizes.get(0)) {
        throw new IllegalArgumentException(
            ("Number of names of y vars is different form size of y in examples: %d vs"
                    + " "
                    + "%d")
                .formatted(xVarNames().size(), xsSizes.get(0)));
      }
  ...
  public static NumericalDataset loadFromCSV(
      InputStream inputStream, List<String> xVarNames, List<String> yVarNames) throws IOException {
  ...
  public static NumericalDataset loadFromCSV(
      InputStream inputStream, String xVarNamePattern, String yVarNamePattern) throws IOException {
    List<Map<String, String>> data = loadFromCSV(inputStream, Long.MAX_VALUE);
  ...
  public static NumericalDataset loadFromCSV(InputStream inputStream, String yVarName)
      throws IOException {
    return loadFromCSV(inputStream, List.of(yVarName));
  }

  public static NumericalDataset loadFromCSV(InputStream inputStream, List<String> yVarNames)
      throws IOException {
    List<Map<String, String>> data = loadFromCSV(inputStream, Long.MAX_VALUE);
    if (!data.get(0).keySet().containsAll(yVarNames)) {
      Set<String> notFoundVars = new LinkedHashSet<>(yVarNames);
  ...
  protected static List<Map<String, String>> loadFromCSV(InputStream inputStream, long limit)
      throws IOException {
    try (inputStream) {
      CSVParser parser =
        ...
        if (lc != 0) {
          if (record.size() != varNames.size()) {
            ...
            Map<String, String> map =
                IntStream.range(0, varNames.size())
  ...
  public static NumericalDataset loadFromCSVResource(
      String name, List<String> xVarNames, List<String> yVarNames) throws IOException {
    return loadFromCSV(ListNumericalDataset.class.getResourceAsStream(name), xVarNames, yVarNames);
  }

  public static NumericalDataset loadFromCSVResource(String name, List<String> yVarNames)
      throws IOException {
    return loadFromCSV(ListNumericalDataset.class.getResourceAsStream(name), yVarNames);
  }

  public static NumericalDataset loadFromCSVResource(String name, String yVarName)
      throws IOException {
    return loadFromCSV(ListNumericalDataset.class.getResourceAsStream(name), yVarName);
  }

  public static NumericalDataset loadFromCSVResource(
      String name, String xVarNamePattern, String yVarNamePattern) throws IOException {
    return loadFromCSV(
        ListNumericalDataset.class.getResourceAsStream(name), xVarNamePattern, yVarNamePattern);
  }
