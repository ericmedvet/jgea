...
public class CooperativeSolver<T1 extends POSetPopulationState<G1, S1, Q>, T2 extends POSetPopulationState<G2, S2, Q>
    , G1, G2, S1, S2, P extends QualityBasedProblem<S, Q>, S, Q> extends AbstractPopulationBasedIterativeSolver<CooperativeSolver.State<T1, T2, G1, G2, S1, S2, S, Q>, P, Void, S, Q> {
  ...
  public State<T1, T2, G1, G2, S1, S2, S, Q> init(
    ...
    QualityBasedProblem<S1, Q> dummyProblem1 = QualityBasedProblem.create(s1 -> null,
        (q1, q2) -> PartialComparator.PartialComparatorOutcome.SAME
    );
    QualityBasedProblem<S2, Q> dummyProblem2 = QualityBasedProblem.create(s2 -> null,
        (q1, q2) -> PartialComparator.PartialComparatorOutcome.SAME
    );
    Collection<Individual<G1, S1, Q>> representatives1 = extractor1.select(solver1.init(dummyProblem1, random, executor)
        .getPopulation(), random);
    Collection<Individual<G2, S2, Q>> representatives2 = extractor2.select(solver2.init(dummyProblem2, random, executor)
        .getPopulation(), random);
    Collection<Individual<Void, S, Q>> evaluatedIndividuals = Collections.synchronizedCollection(new ArrayList<>());
    QualityBasedProblem<S1, Q> problem1 = QualityBasedProblem.create(s1 -> {
      ...
      IntStream.range(0, solutions.size()).forEach(i -> evaluatedIndividuals.add(new Individual<>(
          null,
          solutions.get(i),
          qualities.get(i),
          0,
    ...
    QualityBasedProblem<S2, Q> problem2 = QualityBasedProblem.create(s2 -> {
