...
public interface IterativeSolver<T extends State, P extends Problem<S>, S> extends Solver<P, S> {
  ...
  @Override
  default Collection<S> solve(P problem, RandomGenerator random, ExecutorService executor)
      throws SolverException {
  ...
  default <P2 extends Problem<S>> IterativeSolver<T, P2, S> with(
      Function<P2, P> problemTransformer) {
    ...
    return new IterativeSolver<>() {
      ...
      public Collection<S> extractSolutions(
          ...
          throws SolverException {
        return thisIterativeSolver.extractSolutions(
            problemTransformer.apply(problem), random, executor, state);
      }

      @Override
      public T init(P2 problem, RandomGenerator random, ExecutorService executor)
          throws SolverException {
      ...
      public boolean terminate(
          P2 problem, RandomGenerator random, ExecutorService executor, T state)
          throws SolverException {
        return thisIterativeSolver.terminate(
            problemTransformer.apply(problem), random, executor, state);
      }
