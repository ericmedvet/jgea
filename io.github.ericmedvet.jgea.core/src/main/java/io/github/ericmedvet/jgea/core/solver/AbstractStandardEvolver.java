...
public abstract class AbstractStandardEvolver<
  ...
  protected record State<I extends Individual<G, S, Q>, G, S, Q>(
      ...
      long nOfFitnessEvaluations, PartiallyOrderedCollection<I> pocPopulation
  ...
  protected Collection<G> buildOffspringGenotypes(
      POCPopulationState<I, G, S, Q> state,
    ...
    while (offspringGenotypes.size() < offspringSize) {
      ...
      for (int j = 0; j < operator.arity(); j++) {
        I parent = parentSelector.select(
            (PartiallyOrderedCollection<? extends I>) state.pocPopulation(),
            random
        );
  ...
  protected abstract T update(
      T state, P problem, Collection<I> individuals, long nOfBirths, long nOfFitnessEvaluations
  );
  ...
  protected Collection<I> trimPopulation(
      Collection<I> population, P problem, RandomGenerator random
  ) {
  ...
  public T update(
    ...
    return update(state, problem, newPopulation, nOfBirths, nOfBirths + (remap ? state.pocPopulation().size() : 0));
  }
}
