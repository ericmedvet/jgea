...
public class SymbolicRegressionGrammar extends StringGrammar<String> {
  public SymbolicRegressionGrammar(
      List<Element.Operator> operators, List<String> variables, List<Double> constants) {
    setStartingSymbol("<e>");
    SortedSet<Integer> arities =
        operators.stream()
            .map(Element.Operator::arity)
            .collect(Collectors.toCollection(TreeSet::new));
    List<List<String>> eProductions =
        new ArrayList<>(
            arities.stream()
                .map(
                    a ->
                        Stream.concat(
                                Stream.of(String.format("<o%d>", a)),
                                Collections.nCopies(a, "<e>").stream())
                            .toList())
                .toList());
