...
public class OpenAiEvolutionaryStrategy<S, Q> extends AbstractPopulationBasedIterativeSolver<OpenAiEvolutionaryStrategy.State<S, Q>, TotalOrderQualityBasedProblem<S, Q>, List<Double>, S, Q> {
  ...
  @Override
  public State<S, Q> init(TotalOrderQualityBasedProblem<S, Q> problem, RandomGenerator random, ExecutorService executor) throws SolverException {
    ...
    state.center = IntStream.range(0, n)
        .mapToDouble(i -> genotypes.stream().mapToDouble(genotype -> genotype.get(i)).sum() / genotypes.size()).toArray();
    return state;
  }
  ...
  public void update(
    ...
    List<Individual<List<Double>, S, Q>> individuals = map(genotypes, List.of(), solutionMapper, problem.qualityFunction(), executor, state);
  ...
  private void optimize(State<S, Q> state) {
    ...
    List<Double> weights = IntStream.range(0, batchSize).mapToObj(i -> utilities[2 * i] - utilities[2 * i + 1]).toList();
    ...
    double a = state.stepSize * Math.sqrt(1d - Math.pow(state.beta2, state.getNOfIterations())) / (1d - Math.pow(state.beta1, state.getNOfIterations()));
    state.m = IntStream.range(0, n).mapToDouble(i -> state.beta1 * state.m[i] + (1d - state.beta1 * globalG[i])).toArray();
    state.v = IntStream.range(0, n).mapToDouble(i -> state.beta2 * state.v[i] + (1d - state.beta2 * globalG[i] * globalG[i])).toArray();
    double[] dCenter = IntStream.range(0, n).mapToDouble(i -> -a * state.m[i] / (Math.sqrt(state.v[i]) + state.epsilon)).toArray();
