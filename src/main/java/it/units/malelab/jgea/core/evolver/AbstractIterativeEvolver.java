...
public abstract class AbstractIterativeEvolver<G, S, F> implements Evolver<G, S, F> {
  ...
  public AbstractIterativeEvolver(
  ...
  public static class State implements Evolver.State {
  ...
  protected abstract Collection<Individual<G, S, F>> initPopulation(
      Function<S, F> fitnessFunction, RandomGenerator random, ExecutorService executor, State state
  ) throws ExecutionException, InterruptedException;

  protected abstract Collection<Individual<G, S, F>> updatePopulation(
  ...
  protected static <G1, S1, F1> List<Individual<G1, S1, F1>> map(
      Collection<? extends G1> genotypes,
      Collection<Individual<G1, S1, F1>> individuals,
      Function<? super G1, ? extends S1> solutionMapper,
      Function<? super S1, ? extends F1> fitnessFunction,
      ExecutorService executor,
      State state
  ) throws InterruptedException, ExecutionException {
    List<Callable<Individual<G1, S1, F1>>> callables = new ArrayList<>(genotypes.size() + individuals.size());
    callables.addAll(genotypes.stream().map(genotype -> (Callable<Individual<G1, S1, F1>>) () -> {
      S1 solution = solutionMapper.apply(genotype);
      F1 fitness = fitnessFunction.apply(solution);
      return new Individual<>(genotype, solution, fitness, state.getIterations(), state.getIterations());
    }).toList());
    callables.addAll(individuals.stream().map(individual -> (Callable<Individual<G1, S1, F1>>) () -> {
      S1 solution = solutionMapper.apply(individual.genotype());
      F1 fitness = fitnessFunction.apply(solution);
      return new Individual<>(individual.genotype(), solution, fitness, state.getIterations(),
  ...
  protected Collection<Individual<G, S, F>> initPopulation(
      int n, Function<S, F> fitnessFunction, RandomGenerator random, ExecutorService executor, State state
  ) throws ExecutionException, InterruptedException {
  ...
  public Collection<S> solve(
    ...
    listener.done();
    return new DAGPartiallyOrderedCollection<>(population, individualComparator).firsts()
        .stream()
